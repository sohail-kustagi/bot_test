...
def apply_candle_props(df: pd.DataFrame):
    ...
    df_an['body_perc'] = (df_an.body_size / df_an.full_range) * 100
    df_an['body_lower'] = df_an[['mid_c', 'mid_o']].min(axis=1)
    df_an['body_upper'] = df_an[['mid_c', 'mid_o']].max(axis=1)
    df_an['body_bottom_perc'] = ((df_an.body_lower - df_an.mid_l) / df_an.full_range) * 100
    df_an['body_top_perc'] = 100 - (((df_an.mid_h - df_an.body_upper) / df_an.full_range) * 100)
